...
public class Main {
    public static void main(String[] args) {
        ...
        swiggyStore.addItem(new FoodProduct("Biryani", 100, "Restaurant1"));
        swiggyStore.addItem(new FoodProduct("Noodles", 70, "Restaurant2"));
        swiggyStore.addItem(new FoodProduct("Pasta", 150, "Restaurant1"));
        swiggyStore.addItem(new FoodProduct("Sweet", 30, "Restaurant3"));

        // now I would like to add GroceryItem's to swiggyStore as well, but store only supports adding
        // items of Type IItem. So we create an adapter so GroceryItem can be converted to type IItem
        swiggyStore.addItem(new GroceryItemAdapter(new GroceryProduct("Fruits", 100, "GroceryStore1")));
        swiggyStore.addItem(new GroceryItemAdapter(new GroceryProduct("Vegetables", 70, "GroceryStore2")));
        swiggyStore.addItem(new GroceryItemAdapter(new GroceryProduct("KitchenAppliances", 150, "GroceryStore3")));
        swiggyStore.addItem(new GroceryItemAdapter(new GroceryProduct("Sweet", 30, "GroceryStore1")));

        swiggyStore.showStoreItems();
    }
}